...
public:
	...
	//! @brief Returns the vector with its X and Y elements set to their square roots
	Vector2 sqrt() const;
     ...
     * @see [Reference](https://referencesource.microsoft.com/#System.Numerics/System/Numerics/Vector2.cs "Vector2")
     */
	...
	/**
     * @brief Linear interpolates between @c Value1 and @c Value2, with @c Amount % distance from @c Value1
     * @see [Reference](https://referencesource.microsoft.com/#System.Numerics/System/Numerics/Vector2.cs "Vector2")
     */
     ...
     * @see [Reference](https://referencesource.microsoft.com/#System.Numerics/System/Numerics/Vector2_Intrinsics.cs "Vector2 Intrinsics")
     */
	...
	template <std::size_t I>
	inline std::tuple_element_t<I, Vector2> &get()
	{
		CALCDA_IF_CONSTEXPR (I == 0)
			return x;
		else
...
template <size_t I>
struct CALCDA tuple_element<I, Calcda::Vector2> {
	using type = float;
};

template <>
struct CALCDA hash<Calcda::Vector2> {
	size_t operator()(const Calcda::Vector2 &v) const noexcept
	{
		return Calcda::Internal::hash_combine(hash<float>()(v.x), hash<float>()(v.y));
	}
};
} // namespace std
