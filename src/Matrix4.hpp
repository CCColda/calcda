...
public:
	...
	//! @brief Divides the matrix by @c Other (multiplies by the inverse of @c Other)
     ...
     */
	Matrix4 inverse(Matrix4 *InverseTemporal = nullptr, double *Determinant = nullptr) const;
	...
	//! @brief Multiplies the current matrix with the vector @c Other, then returns the vector result
	Vector4 multiply(const Vector4 &Other) const;

	//! @brief Transposes the current matrix (flips it along its top-left to bottom-right diagonal)
     ...
     */
	static Matrix4 frustum(float Left, float Right, float Top, float Bottom, float Near, float Far);
	...
	/**
     * @brief Returns a perspective projection, with @c Fov in radians, flipped vertically
     *
     * @param Fov Field of View, measured in degrees
     * @param AspectRatio Aspect ratio of the projection
     * @param Near Near plane
     * @param Far Far plane
     */
	static Matrix4 perspectiveFlippedY(float Fov, float AspectRatio, float Near, float Far);

	//! @brief Returns a projection matrix
	static Matrix4 projection(float Left, float Right, float Top, float Bottom, float Near, float Far);

	std::string toString() const;
	std::string toStringO(unsigned int Padding = 0, unsigned int Precision = 2) const;

	//! @brief Identity matrix, 0 everywhere, except the top-left to bottom-right diagonal, where it is 1
