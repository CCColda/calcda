...
Vector2 CALCDA Vector3::xy() const { return Vector2(x, y); }

Vector2 CALCDA Vector3::yz() const { return Vector2(y, z); }

Vector3 CALCDA Vector3::xyz() const { return *this; }

Vector3 CALCDA Vector3::zyx() const { return Vector3(z, y, x); }

float *CALCDA Vector3::getData() { return &x; }
...
Vector3 CALCDA Vector3::operator/(float Amount) const { return divide(Amount); }

Vector3 CALCDA Vector3::operator-() const { return negate(); }
...
Vector3 &CALCDA Vector3::operator/=(float Amount) { return selfDivide(Amount); }
...
float CALCDA Vector3::lengthSquared() const { return x * x + y * y + z * z; }
...
Vector3 CALCDA Vector3::clamp(const Vector3 &Value, const Vector3 &min,
                              const Vector3 &max) {
    float X = Value.x;
    X = (X > max.x) ? max.x : X;
    X = (X < min.x) ? min.x : X;
...
Vector3 CALCDA Vector3::lerp(const Vector3 &Value1, const Vector3 &Value2,
                             float Amount) {
