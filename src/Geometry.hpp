...
public:
	...
	bool doesLineIntersectBoundingRectangle(Vector2 a, Vector2 b, LineType type = LineType::LINE) const;

	virtual bool isPointInside(Vector2 point) const = 0;
	virtual std::vector<Vector2> intersectLine(Vector2 a, Vector2 b, LineType type = LineType::LINE) const = 0;
};
...
public:
	...
	virtual std::vector<Vector2> intersectLine(Vector2 a, Vector2 b, LineType type = LineType::LINE) const final;
	virtual bool isPointInside(Vector2 point) const final;

	std::vector<Vector2> intersectLine(const Line &other) const;

	static std::optional<Vector2> intersectRaw(Vector2 l1a, Vector2 l1b, LineType l1t, Vector2 l2a, Vector2 l2b, LineType l2t);
};
...
public:
	...
	virtual std::vector<Vector2> intersectLine(Vector2 a, Vector2 b, LineType type = LineType::LINE) const override;
};
} // namespace Calcda
